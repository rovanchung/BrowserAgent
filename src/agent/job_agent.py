...
5. If the application asks for a cover letter, call `make_cover_letter` with the job_title, company, location, and full job_description.  Paste/type the returned text directly into the cover letter text field on the page.  If there is NO text field and only a file upload for the cover letter, call the `upload_cover_letter` action to upload the cover letter PDF directly.  Add the cover letter text to your screening answers dict with the exact form label as the key (e.g. "Cover Letter").
...
- For cover letters: prefer pasting text into a text field. If there is only a file upload (no text field), use the `upload_cover_letter` action to upload the PDF.
